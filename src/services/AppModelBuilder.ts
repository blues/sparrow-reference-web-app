...
export interface AppModelBuilder {
     ...
     * @param snapshot 
     */
    buildProjectReadingsSnapshot(snapshot: DomainModel.ProjectReadingsSnapshot): AppModel.Project;


    buildProjectReadingsHistory(history: DomainModel.ProjectHistoricalData): AppModel.Project;
}
...
class ReadingsSnapshotBuilderVisitor implements Visitor {
    ...
    private buildSnapshotReadings<Host extends AppModel.SensorHost>(app: Host, domain: DomainModel.SensorHost) {
        const hostSnapshot = this.snapshot.hostReadings(domain);
        app.currentReadings = this.buildCurrentReadings(hostSnapshot);
         return app;
    }

    buildCurrentReadings(domain: DomainModel.SensorHostReadingsSnapshot): AppModel.SensorTypeCurrentReading[] {
...
class DomainAppModelBuilder implements AppModelBuilder {
    ...
    buildProjectReadingsSnapshot(snapshot: DomainModel.ProjectReadingsSnapshot): AppModel.Project {
        ...
        const project: AppModel.Project = this.buildProject(snapshot.project, visitor);
    ...
    buildNode(node: DomainModel.Node, visitor:Visitor): AppModel.Node {
        return visitor.visitNode({
            ...
            location: this.buildLocation(node.location),
            historicalReadings: null,
            currentReadings: null
        }, node);
    }

    buildLocation(location: string | null): AppModel.TextLocation | null {
        return location==null ? null : { text: location };
    }


    private buildProject(project: DomainModel.Project, visitor: Visitor): AppModel.Project {
    ...
    private buildGateways(gateways: DomainModel.Gateways, visitor: Visitor): AppModel.Gateway[] {
    ...
    private buildDate(date: DomainModel.DomainDate|null): AppModel.AppDate|null {
    ...
    buildReading(reading: DomainModel.Reading | undefined) : AppModel.Reading | null {
