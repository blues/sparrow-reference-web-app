...
export type GatewayWithLatestReadings = Prisma.Gateway & LatestReadingSourceReadings;
...
function loraSignalMetricsToSignalStrengths(metrics?: LoRaSignalMetrics) : SignalStrengths {
  if (!metrics) {
    return "N/A";
  }
...
function sensorReadingsByName(sensors: SensorLatestReadings): LatestReadingsMap {
...
function findReading(readings: LatestReadingsMap, name: string): Prisma.Reading|undefined {
...
function sparrowNodeFromPrismaNode(gatewayUID: string, prismaNode: NodeWithLatestReadings): Node {
...
export const DEFAULT = { sparrowGatewayFromPrismaGateway, sparrowNodeFromPrismaNode };
